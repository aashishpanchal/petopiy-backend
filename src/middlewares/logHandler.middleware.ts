...
morgan.token('status', (_, res) => {
  ...
  if (isBetween(100, 200)) {
    ...
    colorFn = chalk.red; // Red for server error responses
  }
  return colorFn(status);
});

export const logHandler = () => {
  const formate = `:remote-addr - "${chalk.bold(':method :url HTTP/:http-version')}" :status - :response-time ms`;

  return morgan(formate, {
